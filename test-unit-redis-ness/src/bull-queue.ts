...
export class BullTest extends EventEmitter {
  ...
  constructor() {
    super();
    this.queue.on('failed', async (job: Bull.Job, reason: any) => this.failed);
    this.queue.on('completed', async (job: any, status: any) => this.completed);
  }
  private completed = async (job: Bull.Job, status: any) => {
    this.logger.info(`Job ${job.id} completed with status %j`, status);
  }
  private failed = async (job: Bull.Job, reason: any) => {
    this.logger.warn(`Job ${job.id} failed for reason "%j"`, reason);
  }
  public work = (): void => {
    this.logger.debug(`work`);
    this.queue.process('*', async (job: any) => {
      this.logger.debug(`process`);
      this.emit(`jobstart`, job.data);
      return new Promise(async (resolve, reject) => {
        this.logger.info(`Processing job ${job.name}: %s`, job);
  ...
  public queueJob = async (job: IJob): Promise<any> => {
    this.logger.debug(`Adding job: ${job.operation} %j`, job);
    return this.queue.add(job.operation, job);
  }
}
