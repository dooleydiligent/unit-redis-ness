...
describe('mset-command test', () => {
   ...
   */
  it('should create multiple keys at once', async () => {
    response = await sendCommand(client, ['mset', 'key1', 'Hello', 'key2', 'world']);
    expect(response).to.equal('OK');
    response = await sendCommand(client, ['get', 'key1']);
    expect(response).to.equal('Hello');
    response = await sendCommand(client, ['get', 'key2']);
