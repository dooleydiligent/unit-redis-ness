...
describe('lpop-command test', () => {
   ...
   */
  it('should report NIL when lpop called on non-existent key', async () => {
    response = await sendCommand(client, ['lpop', 'no-key']);
    expect(response).to.equal(null);
    response = await sendCommand(client, ['exists', 'no-key']);
    expect(response).to.equal(0);
  });
  it('should return values from the left side of a list', async () => {
    response = await sendCommand(client, ['rpush', testKey, 'a', 'b', 'c', 'd']);
