...
describe('flushdb-command test', () => {
   ...
   */
  it('should remove all keys from only the currently selected databases', async () => {
    ...
    for (let db = 0; db< 16; db++) {
      if (db !== 14) {
        ...
        expect(response).to.equal('OK');
        response = await sendCommand(client, ['exists', 'dbid']);
    ...
    expect(response).to.equal('OK');
    response = await sendCommand(client, ['get', 'dbid']);
