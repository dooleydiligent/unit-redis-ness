...
describe(
    ...
    () => {
        ...
        it(
            "should return ZERO when certain commands are applied against key that doesn't exist",
            async() => {
        ...
        it(
            "should throw an appropriate error when a \"set\" command is applied to a non-set key",
            async() => {
                let response: any = await sendCommand(
                    client,
                    [
        ...
        it(
            "should create a new key when sadd is called correctly",
            async() => {
                let response: any = await sendCommand(
                    client,
                    [
        ...
        it(
            "should only report the number of added keys",
            async() => {
                let response: any = await sendCommand(
                    client,
                    [
        ...
        describe(
            "smove, sismember, and scard tests",
            () => {
                const uniqueKey = `unique${new Date().getTime()}`;
                it(
                    "should do nothing when smove is called with a source key that does not exist",
                    async() => {
                        let response: any = await sendCommand(
                            client,
                            [
                ...
                it(
                    "should move a key from one set to another (and create the target if necessary)",
                    async() => {
                        let response: any = await sendCommand(
                            client,
                            [
                ...
                it(
                    "should report cardinality of a set",
                    async() => {
                        let response: any = await sendCommand(
                            client,
                            [
                ...
                it(
                    "should report ismember true/false",
                    async() => {
                        let response: any = await sendCommand(
                            client,
                            [
                ...
                it(
                    "should only remove the source element if the destination element already exists",
                    async() => {
                        const newkey = `move-unique-${new Date().getTime()}`,
                            newToKey = "move-new-to-key";
