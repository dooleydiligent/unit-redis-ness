...
describe('sinterstore-command test', () => {
  ...
  it('should require a minimum of two parameters', async () =>   {
    response = await sendCommand(client, ['sinterstore', 'ary']);
  ...
  it('should return ZERO when the source set does not exist', async () => {
    response = await sendCommand(client, ['sinterstore', 'dest', 'src']);
  ...
  it('should overwrite the destination and return the number of keys copied', async () => {
    ...
    expect(response).to.equal('OK');
    response = await sendCommand(client, ['sinterstore', 'key3', 'key1', 'key2']);
  ...
  it('should fail when one of the keys is not a set', async () => {
    ...
    expect(response).to.equal(1);
    response = await sendCommand(client, ['sinterstore', 'key1', 'zkey']);
    ...
    expect(response).to.equal('OK');
    response = await sendCommand(client, ['sinterstore', 'key1', 'skey1']);
    expect(response).to.equal('ReplyError: WRONGTYPE Operation against a key holding the wrong kind of value');
  });
});
