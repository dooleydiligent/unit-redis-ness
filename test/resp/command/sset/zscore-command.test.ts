...
describe('zscore-command test', () => {
  ...
  it('should return null when the key or member does not exist', async () => {
    response = await sendCommand(client, ['zscore', 'dontexist', 'two']);
    expect(response).to.equal(null);
    response = await sendCommand(client, ['zscore', 'zset', 'two']);
    expect(response).to.equal(null);
  });
});
