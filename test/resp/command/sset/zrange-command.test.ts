...
describe('zrange-command test', () => {
  ...
  it('should reproduce the redis documentation examples', async () => {
    ...
    expect(response).to.equal(1);
    response = await sendCommand(client, ['zrange', 'zset', '0', '-1']);
    expect(response).to.eql(['one', 'two', 'three']);
    response = await sendCommand(client, ['zrange', 'zset', '2', '3']);
    expect(response).to.eql(['three']);
    response = await sendCommand(client, ['zrange', 'zset', '-2', '-1']);
    expect(response).to.eql(['two', 'three']);
  });
  it('should also reproduce the second documented example', async () => {
