...
export class ZaddCommand implements IRespCommand {
  ...
  public execute(request: IRequest, db: Database): RedisToken {
    ...
    try {
      ...
      const initial: DatabaseValue = db.getOrDefault(zkey, DatabaseValue.EMPTY_ZSET);
  ...
  private parseInput(request: IRequest): DatabaseValue {
    ...
    for (let i = 1; i < request.getParams().length; i += 2) {
      ...
      try {
        const value: string = request.getParam(i);
        ...
        this.logger.warn(`Exception parsing param ${i}`, ex);
